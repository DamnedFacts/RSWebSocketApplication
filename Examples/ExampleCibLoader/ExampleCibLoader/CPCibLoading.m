...
#include "CPCibLoading.h"


@implementation NSBundle (CPCibLoading)
+ (CPCib *)loadCibFile:(CPString)anAbsolutePath externalNameTable:(NSDictionary *)aNameTable
{
...
+ (CPCib *)loadCibNamed:(CPString)aName owner:(id)anOwner
{
    ...
    CPBundle * bundle = anOwner ? [CPBundle bundleForClass:[anOwner class]] : [CPBundle mainBundle];
    NSString *path = [bundle pathForResource:aName];

    return [self loadCibFile:path externalNameTable:@{ CPCibOwner: anOwner }];
}

- (CPCib *)loadCibFile:(CPString)aFileName externalNameTable:(NSDictionary *)aNameTable
{
    return [[[CPCib alloc] initWithContentsOfURL:aFileName] instantiateCibWithExternalNameTable:aNameTable];
}

+ (CPCib *)loadCibFile:(CPString)anAbsolutePath externalNameTable:(NSDictionary *)aNameTable loadDelegate:aDelegate
{
    return ([[CPCib alloc]
...
- (CPCib *)loadCibFile:(CPString)aFileName externalNameTable:(NSDictionary *)aNameTable loadDelegate:(id)aDelegate
{
    return ([[CPCib alloc]
